...
with no special case for index 0, so there is no header and no tile grid. Even taking
the blob as a flat stream of 2-byte rows does not help: 504, 574 and 1804 bytes are
252, 287 and 902 rows, none of them a multiple of 8, so no rgbgfx invocation can
describe these files. Hence this script.
...
encode are exact inverses; `verify` asserts that on the real files.
    ...
    python3 fontgfx.py decode  --cols 2 --height 11 -o out.png in.bin
    python3 fontgfx.py encode  --cols 2 --height 11 -o out.bin in.png
...
def do_verify_all(root):
    ok = True
    for name, (cols, height) in sorted(FONTS.items()):
        binp = os.path.join(root, FONT_DIR, name + ".bin")
        pngp = os.path.join(root, FONT_DIR, name + ".png")
        if not (os.path.exists(binp) and os.path.exists(pngp)):
            print(f"SKIP {name}: need both .bin and .png", file=sys.stderr)
            ok = False
            continue
        ...
        rebuilt = indices_to_bin(pixels_to_indices(pixels, PALETTE), width, cols, height)

        if rebuilt == original:
            print(f"OK   {name}: {len(original)} bytes round-trip exactly")
        else:
            ok = False
            if len(rebuilt) != len(original):
                print(f"FAIL {name}: {len(rebuilt)} bytes rebuilt vs {len(original)}")
            else:
                bad = [i for i in range(len(original)) if original[i] != rebuilt[i]]
                print(
                    f"FAIL {name}: {len(bad)} byte(s) differ, first at "
                    f"0x{bad[0]:X} ({original[bad[0]]:#04x} -> {rebuilt[bad[0]]:#04x})"
                )
    return ok
